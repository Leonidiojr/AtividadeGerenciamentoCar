...
import static application.MainApp.modeloController;
 ...
 * @author leoni
 */
public class frmModelosFind extends javax.swing.JDialog {

    /**
     * Creates new form frmFind
     */
    public frmModelosFind(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        getContentPane().setBackground(Color.WHITE);
        this.setTitle("Cadastrar modelos para " + marcaAtiva.getNome().toString() );
...
public void carregarModelos() {
    ...
    try {
        ...
        SwingWorker<Void, Integer> worker = new SwingWorker<Void, Integer>() {
            ...
            protected Void doInBackground() throws Exception {
                ...
                List<Modelo> ListaModelos = modeloController.pesquisarModelos(jTextField1.getText());
                jProgressBar1.setMinimum(0);
                jProgressBar1.setMaximum(ListaModelos.size());

                // Verifica se a lista de modelos não está vazia
                ...
                // Adiciona os dados de cada modelo na tabela
                for (int i = 0; i < ListaModelos.size(); i++) {
                    // Preenche os dados do modelo na tabela
                    publish(i + 1); // Publica o progresso
                    String[] coluna = new String[5];
                    coluna[0] = String.valueOf(ListaModelos.get(i).getIdModeloPK());
                    coluna[1] = ListaModelos.get(i).getNome();
                    coluna[2] = ListaModelos.get(i).getTipo();
                    coluna[3] = String.valueOf(ListaModelos.get(i).getIdMarca());
    ...
    private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
        ...
        if (selectedRow != -1) { // Verifica se uma linha foi selecionada
            try {
                ...
                Integer idModeloAtivo = null;

                // Verifica o tipo do valor da célula e realiza a conversão adequada
                if (value instanceof String) {
                    idModeloAtivo = Integer.valueOf((String) value);
                } else if (value instanceof Integer) {
                    idModeloAtivo = (Integer) value;
                }

                if (idModeloAtivo != null) {
                    // Realiza a busca pelo modelo ativo
                    modeloAtiva = modeloController.buscarModelo(idModeloAtivo);
                    this.dispose();
                    
                    // Exibe o valor do ID do modelo
                    System.out.println("ID Modelo Ativo: " + idModeloAtivo);
                    
                } else {
                    System.out.println("Valor do ID do modelo é nulo.");
                }
                ...
                System.out.println("Valor do ID do modelo inválido: " + e.getMessage());
                e.printStackTrace();
            } catch (Exception e) {
                // Trata possíveis exceções ao buscar modelo e abrir a janela
                System.out.println("Erro ao buscar modelo ou abrir a janela: " + e.getMessage());
                e.printStackTrace();
            }
        } else {
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        if (this.jButton1.isEnabled()) {
            modeloAtiva = new Modelo(ListaModelos.size()+1, "", 0,"");      
        } else {
            redefineForm();
            jButton1.setEnabled(true);
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(frmModelosFind.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                frmModelosFind dialog = new frmModelosFind(new javax.swing.JFrame(), true);
